...
BGE_PORT = os.environ.get("VLLM_BGE_PORT", "8100")
BGE_MODEL = os.environ.get("BGE_MODEL_NAME", "BAAI/bge-m3")

QWEN3_PORT = os.environ.get("VLLM_QWEN3_PORT", "8101")
QWEN3_MODEL = os.environ.get("QWEN3_MODEL_NAME", "Qwen/Qwen3-Embedding-0.6B")
...
async def get_dense_embeddings(sentences: List[str],
    ...
    url = f"http://localhost:{QWEN3_PORT}/v1/embeddings"

    async with httpx.AsyncClient(timeout=timeout) as client:
        response = await client.post(
            url,
            headers={"Authorization": f"Bearer {API_KEY}"},
            json={"model": QWEN3_MODEL, "input": sentences},
        )
...
async def get_sparse_embeddings(sentences: List[str],
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        tasks = [
            client.post(f"http://localhost:{BGE_PORT}/tokenize",
                       json={"model": BGE_MODEL, "prompt": sentence})
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        result = await client.post(
            f"http://localhost:{BGE_PORT}/pooling",
            json={"model": BGE_MODEL, "input": sentences},
        )
...
def main():
    ...
    print(f"Requesting dense (:{QWEN3_PORT}) and sparse (:{BGE_PORT}) embeddings "
